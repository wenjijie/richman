...
export default {
  ...
  methods: {
    buy() {
      ...
      console.log("step: ", this.players.player1.step);
      console.log("价格：", this.country[this.players.player1.step].price);
      this.players.player1.money -= this.country[
        this.players.player1.step
      ].price;
    ...
    run(step) {
      ...
      setTimeout(() => {
        ...
        // 人物移动动画
        let player = document.getElementById("player");
        let step = this.players.player1.step;
        let end;
        if (step >= 0 && step < 11) {
          end = this.players.player1.position.x - 8.33;
        } else if (step >= 11 && step < 17) {
          end = this.players.player1.position.y - 14.285;
        ...
        let timer = setInterval(() => {
          this.runStep(
            player,
            (this.players.player1.step + random) % 33,
            end,
            timer
          );
        }, 20);
    ...
    runStep(player, endStep, end, timer) {
      ...
      if (step === endStep) {
        ...
        if (this.country[this.players.player1.step].type === "place") {
          this.dialogVisible = true;
        }else if(this.country[this.players.player1.step].type === "fate"){

        }else if(this.country[this.players.player1.step].type === "chance"){
      ...
      // 根据所在位置判断行走方向
      if (step >= 0 && step < 11) {
        this.players.player1.position.x -= 0.5;
        if (this.players.player1.position.x - end < 0.5) {
          player.style.left = end + "%";
          window.clearInterval(timer);
          this.players.player1.step += 1;
          end -= 8.33;
          timer = setInterval(() => {
            if (step === 10) {
          ...
          player.style.left = this.players.player1.position.x - 0.5 + "%";
        }
      } else if (step >= 11 && step < 17) {
        this.players.player1.position.y -= 0.5;
        if (this.players.player1.position.y - end < 0.5) {
          ...
          player.style.top = this.players.player1.position.y - 0.5 + "%";
        }
      } else if (step >= 17 && step < 28) {
        this.players.player1.position.x += 0.5;
        if (end - this.players.player1.position.x < 0.5) {
          player.style.left = end + "%";
          window.clearInterval(timer);
          this.players.player1.step += 1;
          end += 8.33;
        ...
        this.players.player1.position.y += 0.5;
        if (end - this.players.player1.position.y < 0.5) {
          player.style.top = end + "%";
          window.clearInterval(timer);
          if (step >= 33) {
            this.players.player1.step = 0;
          } else {
            this.players.player1.step += 1;
          }
          end += 14.285;
